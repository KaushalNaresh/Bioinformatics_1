...
vector<vector<float>> getProfileMatrix(vector<string> motifs){
    ...
    for(int i = 0; i < motifs[0].size(); i++){
        ...
        profMatrix[0].push_back(countOfA/t);
        profMatrix[1].push_back(countOfT/t);
        profMatrix[2].push_back(countOfG/t);
        profMatrix[3].push_back(countOfC/t);
    }
...
int main(){
    ...
    for(int i = 0; i <= DNA[0].size()-k; i++){
        vector<string> currMotifs = {DNA[0].substr(i, k)};

        for(int j = 1; j < t; j++){
            profMatrix = getProfileMatrix(currMotifs); // 4xk matrix
            currMotifs.push_back(getMostProbableKmer(profMatrix, DNA[j]));
        }
